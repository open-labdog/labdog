...
- [x] **BUG-37** All per-group module sync API endpoints + GitOps task — Celery `.delay()` fires before the `SyncJob` row commits, sporadic `NoResultFound` under load. Surfaced 2026-04-28 from staging on `192.0.2.5` (3 unhandled `NoResultFound` over a 5-min window of 161 successful results); fixed 2026-04-28.
