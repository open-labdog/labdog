...
- [ ] **SEC-11** `backend/app/schemas/git_repos.py:40-46` + `backend/app/packs/git_auth.py:66` — git repo URL accepts loopback/internal hosts (SSRF), HTTPS PAT leaks across redirects

  Symptom: `validate_url_scheme` accepts any URL beginning `https://`, `git@`, or `ssh://` with no host allow-list or RFC1918/loopback/link-local block. Clone runs subprocess `git` with `-c http.extraHeader=Authorization: Bearer <PAT>` (line 66) but does not also set `http.followRedirects=false` — a superuser pointing the URL at an attacker-controlled redirect target exfiltrates the PAT. Severity: **High** (superuser-only attack vector, but the PAT is a long-lived credential and the operator typically trusts labdog with read access to many private repos). Fix: add host allow-list/deny-list to the URL validator; append `-c http.followRedirects=false` to the git invocation when an HTTPS PAT is attached.
