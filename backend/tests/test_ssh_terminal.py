...
class TestSessionRegistry:
    ...
    async def test_get_idle_sessions(self):
        ...
        reg._sessions[sid].last_activity = datetime.now(UTC) - timedelta(seconds=600)
...
class TestWSAuth:
    ...
    async def test_inactive_user_closes_4401(self, db):
        ...
        mock_ws.close.assert_awaited_once_with(code=4401, reason="User not found or inactive")


class TestSSHConnect:
    async def test_host_not_found_raises(self, db):
        from app.ssh_terminal.ssh_connect import HostNotFoundError, open_ssh_shell
    ...
    async def test_no_ssh_key_raises(self, db):
        ...
        from app.ssh_terminal.ssh_connect import NoSSHKeyError, open_ssh_shell
...
class TestSSHTerminalEndpoint:
    ...
    async def test_ws_host_not_found(self, app, db):
        ...
        with (
            patch(
                "app.api.ssh_terminal.get_ws_user",
                new_callable=AsyncMock,
                return_value=fake_user,
            ),
            patch(
                "app.api.ssh_terminal.open_ssh_shell",
                new_callable=AsyncMock,
                side_effect=HostNotFoundError("nope"),
            ),
    ...
    async def test_ws_no_ssh_key_closes_4400(self, app, db):
        ...
        with (
            ...
            patch(
                "app.api.ssh_terminal.open_ssh_shell",
                new_callable=AsyncMock,
                side_effect=NoSSHKeyError("no key"),
            ),
            TestClient(app) as tc,
        ):
            with pytest.raises(Exception):
