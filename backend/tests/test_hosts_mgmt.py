...
from app.hosts_mgmt.merge import render_hosts_file, SYSTEM_ENTRIES
...
class TestHostsSchemas:
    ...
    def test_aliases_validated(self):
        entry = HostsEntryCreate(ip_address="10.0.0.1", hostname="web1", aliases=["web1-alias", "web1.local"])
...
class TestHostsRenderer:
    def test_system_entries_present(self):
        entries = [
            EffectiveHostsEntryResponse(
                ip_address="127.0.0.1", hostname="localhost", aliases=[],
                comment=None, is_system=True, source="system", source_id=0, source_name="system",
            ),
            EffectiveHostsEntryResponse(
                ip_address="::1", hostname="localhost", aliases=["ip6-localhost", "ip6-loopback"],
                comment=None, is_system=True, source="system", source_id=0, source_name="system",
            ),
        ]
    ...
    def test_custom_entry_rendered(self):
        entries = [
            EffectiveHostsEntryResponse(
                ip_address="10.0.0.5", hostname="db.internal", aliases=["db"],
                comment="Database server", is_system=False, source="group", source_id=1, source_name="prod",
            ),
        ]
        rendered = render_hosts_file(entries)
...
class TestHostsDiff:
    def test_in_sync(self):
        ...
        desired = [EffectiveHostsEntryResponse(
            ip_address="10.0.0.1", hostname="web1", aliases=[],
            comment=None, is_system=False, source="group", source_id=1, source_name="g",
        )]
    ...
    def test_hostname_mismatch(self):
        ...
        desired = [EffectiveHostsEntryResponse(
            ip_address="10.0.0.1", hostname="new-name", aliases=[],
            comment=None, is_system=False, source="group", source_id=1, source_name="g",
        )]
        diff = compute_hosts_diff(current, desired)
