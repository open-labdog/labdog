...
READ_ONLY_HEADS: frozenset[str] = frozenset(
    {
        ...
        "cat", "head", "tail", "less", "more", "ls", "ll", "dir", "stat", "file",
        "find", "locate", "readlink", "realpath", "basename", "dirname", "wc",
        "du", "df", "tree", "pwd", "md5sum", "sha256sum", "cksum", "diff", "cmp",
        # text processing (read-only when not redirected; see _has_redirect)
        "grep", "egrep", "fgrep", "zgrep", "cut", "sort", "uniq", "tr", "column",
        "jq", "yq", "strings", "xxd", "od", "base64", "echo", "printf",
        # system state
        "uname", "hostname", "hostnamectl", "uptime", "date", "id", "whoami",
        "who", "w", "last", "lastlog", "groups", "env", "printenv", "locale",
        "lscpu", "lsblk", "lsusb", "lspci", "lsmod", "lsof", "dmidecode",
        "free", "vmstat", "iostat", "mpstat", "sar", "top", "htop", "ps", "pstree",
        "getent", "getconf", "ulimit", "nproc", "arch",
        # networking
        "ip", "ifconfig", "ss", "netstat", "route", "arp", "ping", "ping6",
        "traceroute", "tracepath", "mtr", "dig", "host", "nslookup", "resolvectl",
        "nc", "curl", "wget", "openssl", "nft", "iptables", "ip6tables", "ufw",
        # packages
        "dpkg", "dpkg-query", "apt-cache", "apt-mark", "rpm", "dnf", "yum",
        "zypper", "pacman", "snap", "flatpak", "pip", "pip3", "npm", "gem",
        "needrestart", "debsums",
        # services, logs, containers, virtualisation
        "systemctl", "journalctl", "service", "initctl", "loginctl", "timedatectl",
        "docker", "podman", "nerdctl", "kubectl", "crictl", "virsh", "pvesh",
        "qm", "pct", "zfs", "zpool", "btrfs", "smartctl", "mdadm", "cryptsetup",
        # scheduling and misc
        "crontab", "at", "atq", "sestatus", "getenforce", "aa-status", "ss",
        "true", "false", "test", "which", "type", "command", "whereis", "man",
    }
)
...
MUTATING_SUBCOMMANDS: dict[str, frozenset[str]] = {
    "systemctl": frozenset(
        {"start", "stop", "restart", "reload", "enable", "disable", "mask",
         "unmask", "isolate", "kill", "set-property", "daemon-reload",
         "reboot", "poweroff", "halt", "suspend", "hibernate", "edit", "set-default"}
    ),
    ...
    "docker": frozenset(
        {"run", "rm", "rmi", "start", "stop", "restart", "kill", "exec", "pull",
         "push", "build", "create", "prune", "commit", "cp", "load", "import",
         "update", "compose", "network", "volume", "system", "swarm"}
    ),
    "podman": frozenset(
        {"run", "rm", "rmi", "start", "stop", "restart", "kill", "exec", "pull",
         "push", "build", "create", "prune", "commit", "cp", "load", "import"}
    ...
    "kubectl": frozenset(
        {"apply", "create", "delete", "patch", "replace", "scale", "edit",
         "drain", "cordon", "uncordon", "rollout", "taint", "annotate", "label",
         "exec", "run", "set", "expose", "autoscale"}
    ),
    "crontab": frozenset({"-r", "-e"}),
    "virsh": frozenset(
        {"start", "shutdown", "destroy", "reboot", "reset", "undefine", "define",
         "create", "suspend", "resume", "save", "restore", "setmem", "setvcpus",
         "attach-device", "detach-device", "vol-delete", "pool-destroy"}
    ),
    "qm": frozenset(
        {"start", "stop", "shutdown", "reset", "destroy", "set", "create",
         "rollback", "snapshot", "delsnapshot", "migrate", "resize", "clone"}
    ),
    "pct": frozenset(
        {"start", "stop", "shutdown", "destroy", "set", "create", "rollback",
         "snapshot", "delsnapshot", "migrate", "resize", "clone", "exec"}
    ),
    "zfs": frozenset({"destroy", "create", "set", "rollback", "rename", "receive", "promote"}),
    "zpool": frozenset({"destroy", "create", "add", "remove", "replace", "attach", "detach",
                        "labelclear", "split", "offline", "online"}),
    ...
    "dnf": frozenset({"install", "remove", "erase", "update", "upgrade", "downgrade",
                      "autoremove", "reinstall", "swap"}),
    "yum": frozenset({"install", "remove", "erase", "update", "upgrade", "downgrade",
...
DENYLIST_PATTERNS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (re.compile(r"\b(apt-get|apt|dnf|yum|zypper)\s+(remove|purge|erase)\b[^|;]*\b"
...
def _strip_wrappers(tokens: list[str]) -> list[str]:
    ...
    wrappers = {"sudo", "doas", "nice", "ionice", "nohup", "timeout", "stdbuf",
                "setsid", "time", "env", "command", "builtin", "eval", "exec"}
    idx = 0
