...
async def get_effective_resolver(
    ...
    if host_config:
        return EffectiveResolverResponse(
            ...
            resolver_type=host_config.resolver_type.value if hasattr(host_config.resolver_type, "value") else str(host_config.resolver_type),
    ...
    for group_id, group_name, _priority in groups:
        ...
        if config:
            return EffectiveResolverResponse(
                ...
                resolver_type=config.resolver_type.value if hasattr(config.resolver_type, "value") else str(config.resolver_type),
