...
@celery_app.task(
    bind=True, name="app.tasks.user_sync.user_sync_task", queue="long_running"
)
    ...
    try:
        ...
        async def _run():
            async with task_session() as db:
                ...
                users = [u.model_dump() for u in effective_users]
                groups = [g.model_dump() for g in effective_groups]

                playbook_dict = generate_user_playbook(
                    host.ip_address, users, groups, ssh_key_path
                )
                ...
                desired_state = {"users": users, "groups": groups}
            ...
            async with task_session() as db:
                ...
                if hms is None:
                    hms = HostModuleStatus(
                        host_id=host_id, module_type="linux_user"
                    )
                    db.add(hms)
                hms.sync_status = (
