    ...
    async def _run():
        async with AsyncSessionLocal() as db:
            result = await db.execute(
                select(Host).where(Host.drift_check_enabled == True)
            )
            ...
            for host in hosts:
                try:
                    ...
                    drift_result = await check_drift(host.id, desired)
...
celery_app.conf.beat_schedule = {
    "check-drift-periodic": {
        "task": "app.tasks.drift.check_all_drift",
        "schedule": settings.DRIFT_CHECK_INTERVAL_MINUTES * 60,
    },
}
