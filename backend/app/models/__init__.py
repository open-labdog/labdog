...
**Every model must be reachable from here** — either imported at the top
or listed in ``_EXTERNAL_MODELS``. A model that is neither still works
wherever something happens to import it, and fails everywhere else, which
is how ``ActionRun`` went missing: nothing noticed, because the migrations
are hand-written rather than autogenerated from this metadata, and the
only symptom was a foreign key that could not be resolved in a process
that had imported ``ai_sessions`` but not ``action_runs``.
"""
