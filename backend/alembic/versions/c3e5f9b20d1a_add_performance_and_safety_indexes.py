...
down_revision: Union[str, None] = "a1f3d7c2b0e5"
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_host_module_status_host_id", "host_module_status", ["host_id"]
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_sync_jobs_host_module_status", table_name="sync_jobs"
    )
    op.drop_index(
        "ix_host_module_status_host_id", table_name="host_module_status"
    )
